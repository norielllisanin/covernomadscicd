...
const { roundToTwoDecimals, IHO_HowtoClaim } = require('../../utils');
...
const ihoGenerateQuotationReport = async ({ data }) => {
	...
	doc
		.setFontSize(16)
		.setTextColor(249, 115, 22) // Exact red color (RGB)
	...
	doc.setDrawColor(249, 115, 22); // Set the color for the underline (red)
	doc.line(textX - txtWidth / 2, underlineY, textX + txtWidth / 2, underlineY);
	...
	const total = Number(data.price) + Number(data.vet) || Number(data.price);
	...
	const tableBody = [
		...
		['Total Premium to be paid', `AED ${roundToTwoDecimals(total, 2)}`],
	];
	// Render the table
	...
	doc.autoTable({
		...
		columnStyles: {
			...
			1: { halign: 'center', fontStyle: 'italic' }, // Align second column (Description) to center
		},
	...
	for (let i = 0; i < lines.length; i++) {
		...
		yPosition += lineHeight; // Increase yPosition for next line
	}
	doc
		.setFontSize(10)
		.setTextColor(249, 115, 22) // blue text color
		.setFont(undefined, 'italic')
		.text(
	...
	doc.autoTable({
		...
		bodyStyles: {
			...
			lineColor: [0, 0, 0],
			cellPadding:
				data.productCode === 'VIP'
					? 2
					: data.productCode === 'UAE_INBOUND_20k'
					? 4
					: data.productCode === 'UAE_INBOUND_200k'
					? 3
					: data.productCode === 'SCHPRM' || data.productCode === 'IHOSTD'
					? 2.5
					: 1.5,
		},
		styles: {
		...
		margin: { top: 60, bottom: 35 }, // Set bottom margin to avoid footer overlap
	});
	doc
		.setFontSize(9)
	...
	// Measure the text width and height to match the link area
	...
	// Add the text
	doc
		.setTextColor(249, 115, 22) // Set color to blue (RGB)
	...
	doc
		.setTextColor(249, 115, 22) // Set color to red (RGB)
		.text(text2, linkX2, linkY2);
	...
	while (currentPage <= pageCount) {
		...
		if (currentPage === 2) {
			doc
				.setFontSize(16)
				.setTextColor(249, 115, 22) // Exact blue color (RGB)
				.setFont(undefined, 'italic');
	...
	if (pageCount > 0) {
		...
		while (currentPage <= pageCount) {
			...
			// const textX = 10; // Pehla line thoda upar  15
...
module.exports = ihoGenerateQuotationReport;
