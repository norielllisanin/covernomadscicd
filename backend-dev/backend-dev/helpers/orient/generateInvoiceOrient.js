...
const generateInvoice = async ({ insurerCode, data }) => {
	...
	const policyDetails = [
		[
			{ content: '' }, // empty second column for alignment
			{
				content: testPolicyNumber,
				styles: { textColor: [255, 0, 0], fontStyle: 'bold' }, // ✅ red + bold
			},
		],
		[
			{ content: 'Invoice Number', styles: { fontStyle: 'bold' } },
			{ content: `: ${data.policyNumber.split('-').pop()}` },
		],
		[
		...
		[
			{ content: 'Policy No', styles: { fontStyle: 'bold' } },
			{ content: `: ${data.policyNumber}` },
		],
	];
	...
	// doc.setFont(undefined, 'normal');
	// doc.text(
	// 	'Travel Insurance plans are issued and insured by Orient Takaful PJSC, licensed by the UAE Insurance Authority, Registration number 89. Registered office at Orjowan',
	// 	textX,
	// 	firstLineY,
