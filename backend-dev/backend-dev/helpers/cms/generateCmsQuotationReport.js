...
const { roundToTwoDecimals, CMS_HowtoClaim } = require('../../utils');
...
const generateCmsQuotationReport = async ({ data }) => {
	...
	const policyDetailsData = [
		...
		`${
			data.superSeniorBasePremium === 0
				? ''
				: `INR ${roundToTwoDecimals(Number(data?.superSeniorBasePremium), 2)}`
		}`,
		`${
			data.seniorBasePremium === 0
				? ''
				: `INR ${roundToTwoDecimals(Number(data?.seniorBasePremium), 2)}`
		}`,
		`${
			data.adultBasePremium === 0
				? ''
				: `INR ${roundToTwoDecimals(Number(data?.adultBasePremium), 2)}`
		}`,
		`${
			data.childBasePremium === 0
				? ''
				: `INR ${roundToTwoDecimals(Number(data?.childBasePremium), 2)}`
		}`,
	];
	...
	const tableBody = [
		...
		[
			...
			`INR ${roundToTwoDecimals(Number(data.totalAddOnsPrice), 2)}`,
		],
		['GST @0%', `INR ${roundToTwoDecimals(Number(0), 2)}`],
		[
			'Total Premium to be paid',
			`INR ${roundToTwoDecimals(Number(data.price), 2)}`,
		],
	];
	// Render the table
	doc.autoTable({
		...
		bodyStyles: {
			lineWidth: 0.3,
			lineColor: [0, 0, 0],
			cellPadding: 1, // Add padding in the body cells
			fontSize: 11,
			// fontStyle: 'bold',
		},
		styles: {
			fontSize: 11,
			cellPadding: 1, // Add padding in all cells
	...
	// Add the text
	doc
		.setTextColor(249, 115, 22) //refer to blue
	...
	doc.setTextColor(249, 115, 22).text(text2, linkX2, linkY2);
	...
	if (pageCount > 0) {
		...
		while (currentPage <= pageCount) {
			...
			const heights = [6, 6, 6]; // Different heights for each rectangle
			...
			// First line (normal)
			const textY = pageHeight - 4; // Around center of 10px tall footer
...
module.exports = generateCmsQuotationReport;
